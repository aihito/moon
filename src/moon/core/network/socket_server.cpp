          ...
          [this,
           ...
           conn = std::move(conn)](const asio::error_code& ec, const tcp::endpoint&) {
            ...
            if (!ec) {
              ...
              conn->start(false);
              handle_message(params->owner, message{PTYPE_INTEGER, 0, 0, params->sessionid, conn->fd()});
            }
            else {
  ...
  if (auto iter = udp_.find(host); iter != udp_.end()) {
    ...
    iter->second->sock.async_send_to(
      ...
      [_ = std::move(data), ep, ctx = iter->second](std::error_code ec, std::size_t) {
        if (ec) {
          CONSOLE_ERROR("udp send_to failed %s:%d %s(%d)", ep.address().to_string().data(), static_cast<int>(ep.port()), ec.message().data(), ec.value());
        }
      }
    );
    return true;
