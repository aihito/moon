...
private:
  server* server_;            // Parent server instance
  worker* worker_;            // Worker thread that owns this socket_server
  asio::io_context& context_; // ASIO I/O context for async operations
  asio::steady_timer timer_;  // Timer for connection/socket timeouts

  std::unordered_map<uint32_t, acceptor_context_ptr_t> acceptors_; // fd -> TCP listener (acceptor) contexts
  std::unordered_map<uint32_t, connection_ptr_t> connections_;     // fd -> active TCP connections
  std::unordered_map<uint32_t, udp_context_ptr_t> udp_;            // fd -> UDP socket contexts
};
