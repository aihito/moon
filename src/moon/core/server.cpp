...
{
  send_message(message{PTYPE_TIMER, 0, serviceid, 0, timerid});
}
...
{
  sessionid = -sessionid;
  return send_message({type, sender, receiver, sessionid, std::move(data)});
}

void server::broadcast(uint32_t sender, const buffer& buf, uint8_t type) const
{
  for (auto& w : workers_) {
    w->send(message{type, sender, 0, 0, std::make_unique<buffer>(buf.clone())});
  }
}
...
void server::response(uint32_t to, std::string_view content, int64_t sessionid, uint8_t mtype) const
{
  ...
  send_message(message{mtype, 0, to, sessionid, content});
}
...
std::string server::info() const
{
  ...
  req.append("[\n");
  req.append(moon::format(R"({"id":0, "socket":%zu, "timer":%zu, "log":%zu, "service":%u, "error":%zu})", socket_num(), timer_size, log::instance().size(), service_count(), log::instance().error_count()));
  for (const auto& w : workers_) {
    req.append(",\n");
    req.append(moon::format(R"({"id":%u, "cpu":%f, "mqsize":%u, "service":%u, "timer":%zu, "alive":%u})", w->id(), w->cpu(), w->mq_size(), w->count(), timer_[w->id() - 1]->size(), w->alive()));
  }
  req.append("]");
  return req;
}
