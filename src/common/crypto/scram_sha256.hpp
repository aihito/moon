...
inline std::map<char, std::string> parse_scram_attributes(const std::string& message)
{
  ...
  while (std::getline(ss, segment, ',')) {
    ...
    else if (segment.length() == 1) { // Handle single character flags like 'm' (mandatory extension)
...
public:
  ...
  ScramSha256Client(std::string username, std::string password)
    : username_(username)
    , // REMOVED saslprep_simple_placeholder for testing
    password_(password)
    , // Password is used directly in PBKDF2, not SASLprepped here
  ...
  void process_server_first(const std::string& server_first_message)
  {
    ...
    if (r_it == attributes.end() || s_it == attributes.end() || i_it == attributes.end()) {
      state_ = State::ERROR;
    ...
    if (server_nonce_.rfind(client_nonce_, 0) != 0) { // Check if server nonce starts with client nonce
      state_ = State::ERROR;
  ...
  void process_server_final(const std::string& server_final_message)
  {
    ...
    if (server_signature.size() == expected_server_signature.size() && std::equal(
