...
namespace moon::http
{
using case_insensitive_multimap = std::unordered_multimap<
  std::string,
  std::string,
  moon::ihash_string_view_functor_t,
  moon::iequal_string_view_functor_t>;
using case_insensitive_multimap_view = std::unordered_multimap<
  std::string_view,
  std::string_view,
  moon::ihash_string_view_functor_t,
  moon::iequal_string_view_functor_t>;
...
public:
  ...
  static case_insensitive_multimap parse(std::string_view query_string) noexcept
  {
    ...
    for (std::size_t c = 0; c < query_string.size(); ++c) {
      if (query_string[c] == '&') {
        if (auto name = query_string.substr(
              name_pos,
              (name_end_pos == std::string_view::npos ? c : name_end_pos) - name_pos
            );
            !name.empty()) {
          auto value = value_pos == std::string_view::npos
                         ? std::string_view{}
    ...
    if (name_pos < query_string.size()) {
      ...
      if (!name.empty()) {
        auto value = value_pos >= query_string.size() ? std::string_view{}
      ...
      if (size_t value_start = param_end + 1; value_start < line.size()) {
        ...
        if (value_start < line.size())
          result.emplace(
            ...
            line.substr(value_start, line.size() - value_start)
          );
      }
      line = br.readline();
    }
...
public:
  ...
  static bool parse(
    ...
    case_insensitive_multimap_view& header
  ) noexcept
  {
    buffer_view br(sv.data(), sv.size());
    ...
    if (size_t method_end; (method_end = line.find(' ')) != std::string_view::npos) {
      ...
      if (path_and_query_string_end != std::string_view::npos) {
        ...
        if (size_t protocol_end;
            (protocol_end = line.find('/', path_and_query_string_end + 1)) != std::string_view::npos) {
          if (line.compare(
                path_and_query_string_end + 1,
