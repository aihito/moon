...
--- 从 fd 读一帧；timeout 可选（毫秒）。返回 cmd_id, payload 或 nil, err
function M.read_frame(fd, timeout)
    local head, err = socket.read(fd, 4, timeout)
    ...
    if len > 0 then
        local payload, err2 = socket.read(fd, len, timeout)
